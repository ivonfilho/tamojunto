...
export class EditarOfertasPage implements OnInit {
  ...
  ngOnInit() {
    ...
    if (ofertaId) {
      this.oferta = this.ofertaService.editarOfertaPorId(ofertaId) || {} as Oferta;
      console.log('[ngOnInit] Oferta carregada:', this.oferta);
      
      if (!this.oferta || !this.oferta.id) {
      ...
      // Garantir que preço e desconto sejam números válidos
      ...
      // Inicializar os inputs de texto
      this.precoInput = this.formatBR(this.oferta.preco);
      this.descontoInput = this.formatarParaPorcentagem(this.oferta.desconto);

      console.log('[ngOnInit] Valores formatados - Preço:', this.oferta.preco, 'Desconto:', this.oferta.desconto);
